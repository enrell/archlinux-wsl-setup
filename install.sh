...
EOF
  ...
  if ! grep -Fq "starship init fish | source" "$FISH_CONFIG"; then
    printf "%s\n" "$FISH_CONFIG_CONTENT" >>"$FISH_CONFIG"
    note "Fish configuration has been updated."
  else
    note "Fish configuration already contains the setup."
  fi

  # Determine countries from config (fallback to default) and set alias
  if [ -f /etc/archlinux-wsl-setup.conf ]; then
    # shellcheck disable=SC1091
    . /etc/archlinux-wsl-setup.conf || true
  fi
  printf "\n# Override by installer with selected countries\n" >>"$FISH_CONFIG"
  printf "alias update-mirrorlist=\"sudo reflector -l 30 --country '%s' --age 12 --protocol https,http --save /etc/pacman.d/mirrorlist\"\n" "$COUNTRIES" >>"$FISH_CONFIG"
